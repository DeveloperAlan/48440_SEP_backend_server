...
export const create = ({ bodymen: { body: { email, link } } }, res, next) =>
  User.findOne({ email })
    ...
    .then((reset) => {
      ...
      const content = `
        Hey, ${user.name}.<br><br>
        You requested a new password for your Medical App account.<br>
        ...
        &mdash; Medical App Team
      `
      return sendMail({ toEmail: email, subject: 'Medical App - Password Reset', content })
    })
